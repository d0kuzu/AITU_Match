...
class UserORM:
    ...
    async def update_invites_and_code_by_tgid(tg_id: int, new_invites_count: int, new_invite_code: str):
        async with session_maker() as session:
            print(f"Updating user {tg_id}: invites={new_invites_count}, new_code={new_invite_code}")
            ...
            if updated_user is None:
                print(f"Failed to update user {tg_id} - user not found")
                return False
                
            await session.commit()
            print(f"Successfully updated user {tg_id} in database")
            return True
